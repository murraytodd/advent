...
class TestDay2 extends FunSuite with Matchers {
  ...
  test("Part 1 Samples") {
    compute(0, Vector(1,0,0,0,99)) shouldBe Vector(2,0,0,0,99)
    compute(0, Vector(2,3,0,3,99)) shouldBe Vector(2,3,0,6,99)
    compute(0, Vector(2,4,4,5,99,0)) shouldBe Vector(2,4,4,5,99,9801)
    compute(0, Vector(1,1,1,4,99,5,6,0,99)) shouldBe Vector(30,1,1,4,2,5,6,0,99)
  }

  test("Part 1") {
    val fixed = program.updated(1, 12).updated(2, 2)
    compute(0, fixed)(0) shouldBe 4576384
  }
